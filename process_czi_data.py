...
class ProcessSVSData:
    ...
    def process_data(self):
        ...
        """
        Process CZI  data, merge with inventory, and calculate immunopositivity statistics.

        :return: Final DataFrame with immunopositivity statistics.
        """

        print(f"Processing data for {self.czi_file}...")

        # Open the text file and transform it into a DataFrame
        df_czi = pd.read_csv(self.czi_file, sep='\t', engine='python')
        df_czi['Image'] = df_czi['Image'].str.rstrip('.czi')
        #print("DataFrame from text file:")
        #print(df_czi)
        
        if 'Classification' not in df_czi.columns:
            print(f"Skipping CZI file: {self.czi_file} - 'Classification' column not found.")
            return pd.DataFrame()

        final_df = pd.DataFrame() #Initial empty dataframe

        # Task 1A: Set the different collection for different regions
        unique_parents = df_czi['Parent'].unique()
        dfs_list = []  # To separate the dataframes

        for parent_value in unique_parents:
            # Filter dataframe based on 'Parent' value
            sub_df_czi = df_czi[df_czi['Parent'] == parent_value]
        
            # Start a counter for the Class column
            positive_counter = (sub_df_czi['Classification'] == 'PositiveCell').sum()
            negative_counter = (sub_df_czi['Classification'] == 'NegativeCell').sum()
            ...
            if self.inventory_file.endswith('.csv'):
                ...
                for sheet_name, sheet_df in df_inventory.items():
                    if not sheet_df.empty:
                        ...
                        result = sub_df_czi.merge(sheet_df, left_on='Image', right_on='ID_Slidescanner', how='inner')
                        if not result.empty:
                            #antibody = sheet_name
        ...
        # Create the merged dataframe based on the correspondence between the Image value and the ID_Slidescanner value in the Inventory
                if not merged_df.empty:
                    ...
                    if positive_counter == 0 or negative_counter == 0:
                        print(f"Error: Counter for 'PositiveCell' or 'NegativeCell' is zero for {self.czi_file}. Skipping Positivity Rate calculation.")
                    else:
        ...
        if not dfs_list:
            ...
            print(f"Processed data for {self.czi_file}.")
    ...
    def process_directory(directory_path, inventory_file, output_path, output_ex):
        ...
        for filename in os.listdir(directory_path):
            if filename.endswith('.czi.txt'):
                czi_file = os.path.join(directory_path, filename)
                ...
                #else:
                #    print(f"Skipping file {czi_file} - Processed data is not a DataFrame.")
        ...
        if not valid_dataframes:
            ...
            if no_czi_files:
                print("No files with .czi extension found in the specified directory.")
                sys.exit(1)
