...
class ProcessNDPIData:
    def __init__(self, ndpi_file, inventory_file):

        """
        Initialize the ProcessNDPIData object with NDPI file and inventory file.

        :param ndpi_file: Path to the NDPI data file.
        :param inventory_file: Path to the inventory file (CSV or Excel).
        """

        self.ndpi_file = ndpi_file
    ...
    def process_data(self):
        ...
        """
        Process NDPI data, merge with inventory, and calculate immunopositivity statistics.

        :return: Final DataFrame with immunopositivity statistics.
        """

        print(f"Processing data for {self.ndpi_file}...")

        # Open the text file and transform it into a DataFrame
        df_ndpi = pd.read_csv(self.ndpi_file, sep='\t', engine='python')
        df_ndpi['Image'] = df_ndpi['Image'].str.rstrip('.ndpi')
        #print("DataFrame from text file:")
        #print(df_ndpi)
        
        if 'Classification' not in df_ndpi.columns:
            print(f"Skipping NDPI file: {self.ndpi_file} - 'Classification' column not found.")
            return pd.DataFrame()

        final_df = pd.DataFrame() #Initial empty dataframe

        # Task 1A: Set the different collection for different regions
        unique_parents = df_ndpi['Parent'].unique()
        dfs_list = []  # To separate the dataframes

        for parent_value in unique_parents:
            # Filter dataframe based on 'Parent' value
            sub_df_ndpi = df_ndpi[df_ndpi['Parent'] == parent_value]
        
            # Start a counter for the Class column
            positive_counter = (sub_df_ndpi['Classification'] == 'PositiveCell').sum()
            negative_counter = (sub_df_ndpi['Classification'] == 'NegativeCell').sum()
            ...
            if self.inventory_file.endswith('.csv'):
                ...
                for sheet_name, sheet_df in df_inventory.items():
                    if not sheet_df.empty:
                        ...
                        result = sub_df_ndpi.merge(sheet_df, left_on='Image', right_on='ID_Slidescanner', how='inner')
                        if not result.empty:
                            #antibody = sheet_name
        ...
        # Create the merged dataframe based on the correspondence between the Image value and the ID_Slidescanner value in the Inventory
                if not merged_df.empty:
                    ...
                    if positive_counter == 0 or negative_counter == 0:
                        print(f"Error: Counter for 'PositiveCell' or 'NegativeCell' is zero for {self.ndpi_file}. Skipping Positivity Rate calculation.")
                    else:
        ...
        if not dfs_list:
            ...
            print(f"Processed data for {self.ndpi_file}.")
    ...
    def process_directory(directory_path, inventory_file, output_path, output_ex):
        """
        Process NDPI data from a directory, save antibody-specific data, and return the final DataFrame.

        :param directory_path: Path to the directory containing .ndpi files.
        :param inventory_file: Path to the inventory file (CSV or Excel).
        ...
        for filename in os.listdir(directory_path):
            if filename.endswith('.ndpi.txt'):
                ndpi_file = os.path.join(directory_path, filename)
                print(f"Processing file: {ndpi_file}")
                no_ndpi_files = False
                
                processor = ProcessNDPIData(ndpi_file, inventory_file)
                ...
                #else:
                #    print(f"Skipping file {ndpi_file} - Processed data is not a DataFrame.")
        ...
        if not valid_dataframes:
            ...
            if no_ndpi_files:
                print("No files with .ndpi extension found in the specified directory.")
                sys.exit(1)
    ...
    def process_rate(output_path, final_data_filename):
        ...
        for filename in os.listdir(output_path):
            ...
            if filename.endswith(('.xlsx', 'csv')):
                ...
                #processor = ProcessNDPIData.process_positivity(xlsx_file, final_df)
     ...
     #               print(f"Data columns: {data.columns}")
        ...
        final_files = ProcessNDPIData.merge_samples(final_df, final_data_filename)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process NDPI data with the ProcessNDPIData class")
    parser.add_argument("directory_path", help="Path to the directory containing .ndpi files")
...
#    final_data = ProcessNDPIData.process_directory(directory_path, inventory_file,  output_path)
#    final_rate = ProcessNDPIData.process_rate(output_path, final_data_filename)

#    ProcessNDPIData.process_heatmaps(final_rate)
#    ProcessNDPIData.process_scatterplots(final_rate)
